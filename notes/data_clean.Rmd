...
# add column with MDR, sum across row with TRUE >= 3
ec_interp_eucast <- ec_interp_eucast %>% 
  mutate(MDR = rowSums(.[3:15] == "TRUE")) %>% 
  mutate(
    MDR = ifelse(
      MDR >= 3,
      TRUE,
...
#          MDRO = fun_prevalence(., 15)) %>%
#   select(1, c(16:length(.))) %>%
